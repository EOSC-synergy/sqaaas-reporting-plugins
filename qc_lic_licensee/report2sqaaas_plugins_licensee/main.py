...
class LicenseeValidator(sqaaas_utils.BaseValidator):
    ...
    def validate_qc_lic01(self, license_type, license_file):
        ...
        evidence = None
        standard_kwargs = {"license_type": license_type, "license_file": license_file}
    ...
    def validate_qc_lic02(self, license_type):
        ...
        def check_osi_api(license_type, endpoint):
            OSI_ENDPOINTS = [
                ...
                "https://api.opensource.org.s3.amazonaws.com/licenses/" "licenses.json",
            ]
        ...
        subcriteria = []
        for subcriterion in [
            {"id": "QC.Lic02", "keyword": "approved", "osi_endpoint": "osi-approved"},
            {"id": "QC.Lic02.1", "keyword": "popular", "osi_endpoint": "popular"},
        ]:
            _id = subcriterion["id"]
            ...
            if _id in ["QC.Lic02"] and self.use_spdx:
                ...
                (subcriterion_valid, request_succeed) = is_osi_approved_by_spdx(
                    license_type
                )
            else:
                (subcriterion_valid, request_succeed) = check_osi_api(
                    license_type, _endpoint
                )
            ...
            if not request_succeed:
                ...
                if _use_spdx:
                    _org = "SPDX"
                evidence = "Could not access any of the available %s endpoints" % _org
    ...
    def validate(self):
        criterion = "QC.Lic"
        self.criterion_data = sqaaas_utils.load_criterion_from_standard(criterion)
        subcriteria = []
