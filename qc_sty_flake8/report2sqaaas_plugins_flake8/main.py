...
class Flake8Validator(sqaaas_utils.BaseValidator):
    ...
    def validate(self):
        ...
        if not lines:
            logger.info("No flake8 output has been generated: assuming no errors")
        else:
            for line in lines:
                try:
                ...
                if matches:
                    try:
                        path, row, col, code, text = re.findall(pattern, line)[0]
                    except IndexError:
                        logger.warning(
            ...
            for linting_type, metrics in summary.items():
                logger.info("Linting %s issues found: %s" % (linting_type, metrics))
